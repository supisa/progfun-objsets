...
class Empty extends TweetSet {
  ...
  def mostRetweeted = throw new NoSuchElementException("Empty.mostRetweet")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted = {
    ...
    var mostLeft =
      if(left.elemsCount != 0) max(elem,left.mostRetweeted) else elem
    if(right.elemsCount != 0) max(mostLeft,right.mostRetweeted) else mostLeft
  }
  def descendingByRetweet = {
    val most = left.union(right).incl(elem).mostRetweeted
    new Cons(most,left.union(right).incl(elem).remove(most).descendingByRetweet)
  }
  ...
  def foreach(f: Tweet => Unit): Unit = {
    ...
    def elemsCount = 1 + left.elemsCount + right.elemsCount
}
